 ...
 *
 */
public class WhatsAppDialog extends Window implements EventListener<Event>, ValueChangeListener
{
	/**
	 * 
	 */
	 ...
	 */
	public WhatsAppDialog (String title, MUser from, String to, 
		String subject, String message, DataSource attachment)
	{
		...
		if (!ThemeManager.isUseCSSForWindowSize())
		{
			...
			ZKUpdateUtil.setHeight(this, "50%");
		}
		this.setShadow(true);
	...
	/**	Logger			*/
	private static CLogger log = CLogger.getCLogger(WhatsAppDialog.class);
...
//		fMessage.setCaretPosition(0);
	...
	public void onEvent(Event event) throws Exception {		
		...
		else if (event.getTarget().getId().equals(ConfirmPanel.A_OK))
		{
			...
			String whatsappapiURLFile = MSysConfig.getValue("WhatsappAPI") 
					+ MSysConfig.getValue("WhatsappInstanceId")
			...
			sendMessage(whatsappapiURLFile, fileMsg);
	...
	private String getBase64PDFString() {
		String pdfBase64 = null;
		FileDataSource fs = (FileDataSource) getAttachment();
		byte[] fileContent;
		try {
			fileContent = FileUtils.readFileToByteArray(fs.getFile());
			pdfBase64 =  Base64.getEncoder().encodeToString(fileContent); //javax.xml.bind.DatatypeConverter.printBase64Binary(fileContent);
		} catch (IOException e) {			
			e.printStackTrace();
		}
	...
	private void sendMessage(String apiUrl, HashMap<String, String> data) {
		String result;
